...
describe("shared/orders/order-service.js", () => {
    ...
    test("re-reads the latest order before applying a collection confirmation to avoid lost updates", async () => {
        // Simulate the race where the customer's page holds a stale snapshot
        // (vendorConfirmedCollected = false locally), but the vendor has
        // already flipped vendorConfirmedCollected = true in Firestore. The
        // service should pick up the fresh DB state and finalize the order
        ...
        const freshFirestoreFns = createFirestoreFns({
            includeUpdateDoc: true,
            getDocResult: createDocSnapshot("order-race", {
                customerUid: "customer-1",
                customerName: "Tshepo",
                customerEmail: "tshepo@example.com",
                vendorUid: "vendor-1",
        ...
        expect(result.order.vendorConfirmedCollected).toBe(true);
        expect(freshFirestoreFns.getDoc).toHaveBeenCalled();
    });
