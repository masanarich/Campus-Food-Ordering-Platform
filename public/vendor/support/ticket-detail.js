(function attachVendorSupportTicketDetailPage(globalScope) {
    "use strict";

    const MODULE_NAME = "vendor/support/ticket-detail";
    ...
    function getFallbackRoutes() {
        return { list: "./index.html", newTicket: "./new.html", dashboard: "../index.html" };
    }
    ...
    function isTicketClosed(ticket, options = {}) {
        ...
        return normalizeLowerText(status) === "closed";
    }

    function canReporterReopen(ticket) {
        return normalizeLowerText(ticket && ticket.status) === "closed";
    }

    function canReporterClose(ticket) {
    ...
    function hasSupportReply(replies) {
        return (Array.isArray(replies) ? replies : []).some(function matchSupportReply(reply) {
            ...
            return safeReply.isInternalNote !== true && (role === "admin" || role === "customer");
        });
    }

    function getNextVendorTicketAction(ticket, replies = []) {
        ...
        if (!hasSupportReply(replies) && replyCount === 0) {
            return {
                ...
                detail: "Your ticket is open. Support will reply here when they pick it up."
            };
        }

        return {
            label: "Next: add business details if needed",
            detail: "Review the conversation and reply if support needs more shop, order, or payout context."
        };
    }
    ...
    function renderTicketSummary(elements, ticket, options = {}) {
        ...
        const nextAction = getNextVendorTicketAction(ticket, options.replies);
        ...
        const categoryTone = ticketFormatters && typeof ticketFormatters.getTicketCategoryTone === "function"
            ? ticketFormatters.getTicketCategoryTone(ticket.category, options.ticketCategories) : "info";
        ...
        const priorityTone = ticketFormatters && typeof ticketFormatters.getTicketPriorityTone === "function"
            ? ticketFormatters.getTicketPriorityTone(ticket.priority) : "neutral";
        ...
        setOutputText(elements.reporter, reporterText, "Vendor");
    ...
    function renderReplies(container, emptyMessage, replies, options = {}) {
        ...
        const entries = ticketFormatters && typeof ticketFormatters.formatReplyEntries === "function"
            ? ticketFormatters.formatReplyEntries(replies, { ...options, includeInternalNotes: false })
            : (Array.isArray(replies) ? replies.filter((r) => !(r && r.isInternalNote === true)) : []);
    ...
    function applyTicketStatusToActions(elements, ticket, options = {}) {
        ...
        const canClose = canReporterClose(ticket);
        const canReopen = canReporterReopen(ticket);
        ...
        if (elements.replyFormSection) {
            elements.replyFormSection.setAttribute("data-state", closed ? "read-only" : "open");
        }
    }
    ...
    async function loadTicketAndReplies(options = {}) {
        ...
        try {
            ...
            // The vendor detail page must only ever surface tickets the signed-in user
            // raised from the vendor side. A user who is both a vendor and a customer
            // can have tickets under the same UID with reporterRole === "customer";
            // those belong to the customer portal and must not leak across.
            if (
    ...
    async function changeTicketStatus(options = {}) {
        ...
        try {
            ...
            return { success: false, error: { code: "unknown-action", message: `Unknown ticket action: ${action}.` } };
        } catch (error) {
            return {
                success: false,
                error: {
    ...
    function attachReplyHandler(elements, options) {
        ...
        if (!form || form.dataset.vendorReplyBound === "true") return null;
        form.dataset.vendorReplyBound = "true";
        ...
        async function handleSubmit(event) {
            ...
            const result = await submitReply({ ...options.runtime(), body });
    ...
    function attachActionHandlers(elements, options) {
        ...
        if (closeButton && closeButton.dataset.vendorCloseBound !== "true") {
            closeButton.dataset.vendorCloseBound = "true";
            handlers.closeHandler = async function handleClose(event) {
                ...
                const result = await changeTicketStatus({ ...options.runtime(), action: "close" });
        ...
        if (reopenButton && reopenButton.dataset.vendorReopenBound !== "true") {
            reopenButton.dataset.vendorReopenBound = "true";
            handlers.reopenHandler = async function handleReopen(event) {
                ...
                const result = await changeTicketStatus({ ...options.runtime(), action: "reopen" });
    ...
    function initializeVendorSupportTicketDetailPage(options = {}) {
        return init(options);
    }

    const vendorSupportTicketDetailPage = {
        MODULE_NAME, REPLY_MIN_LENGTH, REPLY_MAX_LENGTH,
        ...
        getFallbackRoutes, getTicketIdFromQuery, waitForAuthReady,
        setStatusMessage, setOutputText, toggleHidden, setSubmitButtonState,
        isTicketClosed, canReporterReopen, canReporterClose,
        hasSupportReply, getNextVendorTicketAction,
        renderTicketSummary, renderTimeline, renderReplies,
        applyTicketStatusToActions, loadTicketAndReplies, renderTicket,
        submitReply, changeTicketStatus,
        attachReplyHandler, attachActionHandlers, collectElements,
        init, initializeVendorSupportTicketDetailPage
    };

    if (typeof module !== "undefined" && module.exports) module.exports = vendorSupportTicketDetailPage;
    if (typeof globalScope !== "undefined") globalScope.vendorSupportTicketDetailPage = vendorSupportTicketDetailPage;
